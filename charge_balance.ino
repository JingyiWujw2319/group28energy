...
float soc = 0, soc1, soc2, soc_init;
float cap_delta;
float soc_delta;
...
void loop() {
  if (loop_trigger == 1) { // FAST LOOP (1kHZ)
    ...
    if ((state_num == 0) || (state_num == 2) || (state_num == 4) || (state_num == 6) || (state_num == 9) || (state_num == 8) || (state_num == 7)) { //when resting
    ...
    } else if (state_num == 10) {//charge bat2
      digitalWrite(9, false);//turn on bat2
      digitalWrite(4, true);//keep bat1 off
      V_Bat1 = analogRead(A1) * 4.096 / 1.03; //Mea1
      V_Bat = analogRead(A0) * 4.096 / 1.03;
      V_Bat2 = 0;
    } else if (state_num == 11) { //charge bat1
      digitalWrite(4, false);//turn on bat1
      digitalWrite(9, true);//keep bat2 off
      V_Bat = analogRead(A0) * 4.096 / 1.03;
    ...
    soh = 0; // no useful values of soh during charging process
    aged = 0;
    t_remain = soc*2.688;//estimate remain working time during charging
  ...
  if (int_count == 1000) { // SLOW LOOP (1Hz)
    ...
    switch (state_num) { // STATE MACHINE (see diagram)
      case 0: { // Start state (no current, no LEDs)
          ...
          if(V_Bat1 > V_Bat2){//take larger value: while balancing, the other is just catching up with the higher voltage bat
            soc = soc1;
          }else{
            soc = soc2;
          }
      ...
      case 1: { //Charge state
          soc = soc + (current_measure / 3808800) * 100; //2 bats total cap=561+497=1058mAh
//          current_ref = 250;
          
          //for <3500 mV charging is slow, so can check less frequently
      if(soc < 100){
          if (V_Bat < 3500) { // if not charged, stay put//      soc < 95   -------------------------------------------------------
            if (charge_timer < 1200) { // charging for 20min
              next_state = 1;
              digitalWrite(8, true);
              charge_timer++;
      ...
      case 2: { // Charge Rest, green LED is off and no current
          ...
          soc = soc;
          //soc = 100;
          if (rest_timer < 30) { // Stay here if timer < 1min
            next_state = 2;
            digitalWrite(8, false);
            rest_timer++;
...
//      case 7: { //final balancing when reach 3600 VBat
//          current_ref=0;
//          digitalWrite(4, true);//Rly1 on
//          digitalWrite(9, true);//Rly2 on
//          V_Bat1 = analogRead(A1) * 4.096 / 1.03; //Mea1
//          V_Bat2 = analogRead(A2) * 4.096 / 1.03; //Mea2
//
//          if (rest_timer < 60) { //rest for 1 min before final balancing, if abs(V_Bat1- V_Bat2) > 50
//            next_state = 7;
//            rest_timer ++;
//          } else {
//            if (abs(V_Bat1 - V_Bat2) > 5) { //too large difference, go to normal balancing
//              next_state = 8;
//            } else { // if small difference, discharge the higher voltage one
//              next_state = 2;
//            }
//            rest_timer = 0;
//          }
...
//        }
      ...
      case 8: { // measure initial ocv of each cell
          current_ref=0;
          digitalWrite(4, true);//Rly1 on
          digitalWrite(9, true);//Rly2 on
          V_Bat1 = analogRead(A1) * 4.096 / 1.03; //Mea1
      ...
      case 10: { // charge bat2 for 3min each time and compare
          digitalWrite(9, false);//turn on bat2
          digitalWrite(4, true);//keep bat1 off
          V_Bat1 = analogRead(A1) * 4.096 / 1.03; //Mea1
          V_Bat2 = 0;
//          current_ref = 250;
          ...
          soc2 = soc2 + (current_measure / 2012400) * 100;//bat2 cap = 559mAh
          soc1 = soc1;

          if ( charge_timer < 300) { //charge bat2 with whole current for 5min        //--------------------------
            next_state = 10;
            charge_timer++;
      ...
      case 11: { // charge bat1 with 250mA
          digitalWrite(4, false);//turn on bat1
          digitalWrite(9, true);//keep bat2 off
          V_Bat2 = analogRead(A2) * 4.096 / 1.03; //Mea2
          V_Bat1 = 0;
//          current_ref = 250;
          ...
          soc1 = soc1 + (current_measure / 2019600) * 100;//bat2 cap = 561mAh
          soc2 = soc2;


          if (charge_timer < 300) { // charge Bat1 with whole current for 5min          //--------------------------------
            next_state = 11;
            charge_timer ++;
      ...
      case 12: { // cc Charge state fast check
//        current_ref = 250;
        soc = soc + (current_measure / 4032000) * 100; //2 bats total cap=561+559=1120mAh


          if (soc < 100) { // if not charged, stay put
            if (fast_timer < 300) { //check every 5min
              next_state = 12;
              digitalWrite(8, true);
              fast_timer ++;
            } else {
          ...
          } else { //go to charge rest directly
            next_state = 2;
          }
...
//    dataString1 = String(state_num) + "," + "V_Bat=" + String(V_Bat) + "," + "Iref=" + String(current_ref) + "," + " Imeasure=" + String(current_measure) + "," + " soc=" + String(soc) + "," + " soc1=" + String(soc1)+ "," + " soc2=" + String(soc2)+ "," + "V_Bat1=" + String(V_Bat1) + "," + "V_Bat2=" + String(V_Bat2) + "," + "duty=" + String(pwm_out) + "power=" + String(power_now); //build a datastring for the CSV file
    dataString1 = "<<," +  String(soc) + "%," + String(soh) + "%," + String(aged) + ",Charging" + "," + String(t_remain)+ "min" ;
    dataString2 = String(state_num) + "," + String(V_Bat) + "," + String(current_ref) + "," + String(current_measure) + "," + String(soc) + "," + String(V_Bat1) + "," + String(V_Bat2) + "," + String(pwm_out)+ String(power_now);
